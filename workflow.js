...
function renderWorkflow(){
  ...
  if(acked && res){
    ...
    rosterHtml = `<div class="panel wide">
...
function wireWorkflow(root){
  ...
  on("[data-wfrun]",    ()=>{ runFunnelStage(F.stage); render(); });
  on("[data-wfnext]",   ()=>{ F.stage=Math.min(F.stage+1, FUNNEL_STAGES.length-1); saveFunnel(); render(); window.scrollTo(0,0); });
