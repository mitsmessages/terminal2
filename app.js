...
function renderTearsheet(t){
  ...
  <button class="backbtn backbtn-float" id="backBtnFloat" title="Back to ${State.returnTab&&State.returnTab!=="stocks"?State.returnTab:"stock list"}">← Back</button>
...
async function sendAIMessage(t, text){
  ...
  try{
    let body = {system_instruction:{parts:[{text:sys}]}, contents, tools:[{google_search:{}}]};
    let r = await call(body);
    if(!r.ok){ delete body.tools; r = await call(body); }   // some keys don't allow the search tool
...
function wireEvents(){
  ...
  if(backBtnFloat) backBtnFloat.onclick=goBack;
